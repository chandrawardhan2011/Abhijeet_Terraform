...
def check_identity(cfg: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if is_windows():
        ...
        # Windows prints these values with a colon, e.g.:
        #   Minimum password length: 12
        #   Lockout threshold: 5
        #   Length of password history maintained: 5
        # Older regex without ':' makes the agent falsely report VULNERABLE.
        min_len = re.search(r"Minimum password length\s*:\s*(\d+)", acc, re.I)
        hist = re.search(r"Length of password history maintained\s*:\s*(\d+|None)", acc, re.I)
        lockout = re.search(r"Lockout threshold\s*:\s*(\d+|Never)", acc, re.I)

        min_len_value = int(min_len.group(1)) if min_len and min_len.group(1).isdigit() else 0
        hist_value = int(hist.group(1)) if hist and hist.group(1).isdigit() else 0
        lockout_value = int(lockout.group(1)) if lockout and lockout.group(1).isdigit() else 0

        out.append(result("PASSWORD_MIN_LENGTH_STRONG", "PATCHED" if min_len_value >= 12 else "VULNERABLE", "Windows minimum password length", acc))
        out.append(result("ACCOUNT_LOCKOUT_ENABLED", "PATCHED" if lockout_value >= 1 else "VULNERABLE", "Windows account lockout threshold", acc))
        out.append(result("PASSWORD_HISTORY_ENFORCED", "PATCHED" if hist_value >= 5 else "VULNERABLE", "Windows password history", acc))
    elif is_linux():
