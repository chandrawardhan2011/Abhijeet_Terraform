...
VULN_DB: Dict[str, dict] = {
    ...
    "cronjob_misconfig":         {"id":"cronjob_misconfig","label":"LS04",        "name":"Cronjob Misconfiguration",              "cve":"CWE-732",      "severity":"high",     "vm_types":["linux","ubu","web","db","ftp"]},
    "weak_file_permissions":     {"id":"weak_file_permissions","label":"LS07",    "name":"Weak File Permissions",                 "cve":"CWE-732",      "severity":"medium",   "vm_types":["linux","ubu","web","db","ftp","wazuh"]},
    "unpatched_kernel":          {"id":"unpatched_kernel","label":"LS08",         "name":"Unpatched Kernel / ASLR Disabled",      "cve":"CWE-693",      "severity":"critical", "vm_types":["linux","ubu"]},
    "suid_binary":               {"id":"suid_binary","label":"LS09",              "name":"SUID Binary Misconfiguration",          "cve":"CWE-250",      "severity":"high",     "vm_types":["linux","ubu","web","db","ftp"]},
    "nfs_misconfiguration":      {"id":"nfs_misconfiguration","label":"LS10",     "name":"NFS no_root_squash Misconfiguration",   "cve":"CWE-732",      "severity":"high",     "vm_types":["linux","ubu","db"]},
    "plain_text_creds":          {"id":"plain_text_creds","label":"LS11",         "name":"Plaintext Credentials in Config",       "cve":"CWE-312",      "severity":"high",     "vm_types":["linux","ubu","web","db"]},
    "open_ports":                {"id":"open_ports","label":"LS12",               "name":"Unnecessary Open Ports / No Firewall",  "cve":"CWE-284",      "severity":"medium",   "vm_types":["linux","ubu","web","db","ftp","wazuh"]},
    "log_tampering":             {"id":"log_tampering","label":"LS13",            "name":"Log Tampering / Audit Disabled",        "cve":"CWE-778",      "severity":"medium",   "vm_types":["linux","ubu","web","db","ftp"]},
    ...
    "world_writable_dirs":       {"id":"world_writable_dirs","label":"LC03",      "name":"World-Writable Dirs with Sensitive Data","cve":"CWE-732",     "severity":"medium",   "vm_types":["linux","ubu"]},
    ...
    "passwd_writable":           {"id":"passwd_writable","label":"LC02",          "name":"World-Writable /etc/passwd",            "cve":"CWE-732",      "severity":"critical", "vm_types":["linux","ubu"]},
    ...
    # ── Scenario Scripts ─────────────────────────────────────────────────────
    "scenario_2":                {"id":"scenario_2","label":"SC02",               "name":"Scenario 2 — TryHackMe Lab Setup",      "cve":"CWE-284",      "severity":"critical", "vm_types":["windows","win10"]},
    "scenario2linux":            {"id":"scenario2linux","label":"SC03",            "name":"Scenario 2 Linux — TryHackMe Lab",      "cve":"CWE-284",      "severity":"critical", "vm_types":["ubu"]},
}
...
async def websocket_patch(ws: WebSocket):
    ...
    if is_windows:
        script_basename   = os.path.basename(patch_script)
        local_script_path = os.path.join(ANSIBLE_DIR, patch_script)
        # Use forward slashes — Windows PowerShell accepts them fine
        remote_path = "C:/Temp/{}".format(script_basename)

        playbook_lines = [
            "---",
            "- name: Patch Windows target",
            "  hosts: {}".format(target_host),
            "  gather_facts: false",
            "  tasks:",
            ...
            "        src: '{}'".format(local_script_path),
            "        dest: '{}'".format(remote_path),
            "",
            "    - name: Execute patch script",
            "      ansible.windows.win_shell: >",
            "        powershell.exe -ExecutionPolicy Bypass -NonInteractive -File '{}'".format(remote_path),
            "      register: patch_out",
            "",
        ...
        import tempfile
        tmp_pb = tempfile.NamedTemporaryFile(
            mode="w", suffix=".yml", dir=ANSIBLE_DIR,
            prefix="patch_win_", delete=False
        )
        tmp_pb.write(playbook_content)
        tmp_pb.flush()
        tmp_pb.close()
        tmp_pb_path = os.path.basename(tmp_pb.name)
        ...
        cmd = ["stdbuf", "-oL", "ansible", target_host,
               ...
               "-a", patch_script,
               "--become"]
        tmp_pb_path = None   # no temp file for Linux
    ...
    process.wait()
    # Clean up temp playbook file created for Windows patch
    if is_windows and tmp_pb_path:
        try: os.remove(os.path.join(ANSIBLE_DIR, tmp_pb_path))
        except: pass
    success = process.returncode == 0
    try:
        await ws.send_json({
