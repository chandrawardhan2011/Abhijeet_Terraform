...
provider "proxmox" {
  pm_api_url                  = "https://50.50.50.200:8006/api2/json"
...
resource "proxmox_vm_qemu" "web" {
  count = var.web_count
  depends_on=[proxmox_vm_qemu.wazuh]
...
disk{
  ...
  ipconfig0       = "ip=10.0.20.${21 + count.index}/24,gw=10.0.20.1"
  ciuser          ="ansible"
  cipassword      ="abc@123"
  ciupgrade       = false
  agent           = 1
...
resource "proxmox_vm_qemu" "db" {
  count = var.db_count
  depends_on=[proxmox_vm_qemu.web]
...
resource "proxmox_vm_qemu" "kali" {
  ...
  depends_on=[proxmox_vm_qemu.ftp]
...
# ─────────────────────────────────────────────────────────────────────────────
# WINDOWS CLIENTS — VLAN 20 (10.0.20.15x) — VMIDs starting 2000
# ─────────────────────────────────────────────────────────────────────────────

resource "proxmox_vm_qemu" "win10" {
  count = var.win10_count
  depends_on=[proxmox_vm_qemu.kali]
 ...
 disk{
  ...
  ipconfig0  = "ip=10.0.20.${150 + count.index}/24,gw=10.0.20.1" 
  ciupgrade  = false
  ciuser     = "ansible"
  cipassword = "abc@123"
}
...
resource "proxmox_vm_qemu" "linux" {
  ...
  depends_on=[proxmox_vm_qemu.win10]
